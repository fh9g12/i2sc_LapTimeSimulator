...
%                                straight through to open.naca4Aero
%                                (InvertWing=true by default there), so
%                                this is exactly open.naca4Aero's own
...
%                  Both wings are analysed via open.naca4Aero (its
...
% back as NaN from open.naca4Aero and propagates through to CL, CD and
...
% (frontAoA,rearAoA) combinations can just check for NaN afterward.
    ...
    [ClF,CdF] = open.naca4Aero(frontSection,frontAoA,'chord_m',options.frontChord_m) ;
    [ClR,CdR] = open.naca4Aero(rearSection,rearAoA,'chord_m',options.rearChord_m) ;
